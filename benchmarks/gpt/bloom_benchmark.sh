...
# request_batch_size_list=(1 8)
# input_length_list=(128 512)
# request_output_len_list=(8 32)
request_batch_size_list=(1 8 16 32)
input_length_list=(4096)
request_output_len_list=(8 32 128)

logdir="bloom-TP${tensor_para_size}-PP${pipeline_para_size}-log"
if [ ! -f "${logdir}" ]; then
  mkdir "${logdir}" -p
fi
...
model_size_list=("176b")
for model_size in "${model_size_list[@]}"; do
  if [ "$model_size" = "560m" ]; then
    head_num=16
    size_per_head=64
        ...
        for request_output_len in "${request_output_len_list[@]}"; do
          for decode_value in "${decode_values[@]}"; do
            ...
            python ../examples/pytorch/gpt/utils/generate_gpt_config.py \
              ...
              --model_dir "../models/huggingface-models/c-model/bloom-${model_size}/${total_gpu_count}-gpu/" \
